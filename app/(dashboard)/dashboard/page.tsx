...
import { getDashboard, getCareerMomentum, getUpcomingTasks, getStreak } from "@/services/dashboardService"
...
export default function DashboardPage() {
  ...
  const handleTaskCompleted = (taskId: number) => {
    ...
    // Optimistically reflect "activity today" so the streak card
    // updates immediately instead of waiting for the next full load.
    setStreak((prev) => {
      if (!prev || prev.activeToday) return prev
      return {
        ...prev,
        activeToday: true,
  ...
  if (!dashboard && !dashboardLoading) {
    return <div className="p-4 sm:p-6">Failed to load dashboard data.</div>
  }
